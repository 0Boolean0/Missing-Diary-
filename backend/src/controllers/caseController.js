...
<<<<<<< HEAD
    // Admin: see all cases, optionally filtered by status
...
<<<<<<< HEAD
    // Police: always see only active/verified cases
...
export async function approveCase(req, res, next) {
  try {
    // Approve sets status to 'active' so the case becomes publicly visible
    const result = await query(
